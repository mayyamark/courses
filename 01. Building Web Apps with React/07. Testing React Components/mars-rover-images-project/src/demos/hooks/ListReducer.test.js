// Testing Components With State And Effects (with React Testing Library) - Testing The useReducer Hook
...
describe("ListReducer", () => {
  // same as the List.test.js since it is a black box :)
  ...
  describe("reducer", () => {
    ...
    describe("add items", () => {
      it("should add the items", () => {
        let state = reducer({ items: [], newItem: "aurora" }, { type: "add" });
        expect(state.items).toEqual(["aurora"]);
        state = reducer({ ...state, newItem: "borealis" }, { type: "add" });
